...
// 01/26/2017   rjw 1.7 changed "C" to pump on add pumpOnAdditional ( 60secs ) to timeout
// 01/26/2017   rjw 1.7 changed "D" to pumpSTOP = no reverse
// ----------------------------------------------------------------------------------------------------
...
// --------------------------------------------------------------------
// A       OFF All     a         All off         OFF Pump, LED, Pump REVERSES
// B       ON All      b         All on          On Pump, LED, on until pump timeout or A or D reeceived
// C       ON Pump     c         Pump on         Pump runs for pumpOnLimit + PumpOnAdditional or A or D received
// D       OFF Pump    d         Pump off        Pump Stops NO REVERSE
// E       ON LED      e         LED on
// F       OFF LED     f         LED off
...
unsigned long pumpOnAdditional  = 0;         // add more time to pumpOnLimit = pumpRevTime + 500
...
char processCommand(char c1) {
  switch (c1) {
    ...
    case 'C':
      PumpOnFWD();
      pumpOnAdditional  = 57500; //  add a minute to the pump on limit
      return 0;
      break;
...
void loop()
{
  ...
  if ((pumpOn) and !(ManualMode)) {
    if (millis() - pumpOnMillis > (pumpOnLimit + pumpOnAdditional)) {
      PumpStop(); // TURN THE PUMP OFF
      Serial.write("due to timeout\n");
      ManualModeOff();          // rest manual mode wait time
    }
  }
  ...
  if (currentMillis - previousMillis > interval / divisor) {
    ...
    if (!(ManualMode)) {
      digitalWrite(pb_lt, !digitalRead(pb_lt));
    }
  }
